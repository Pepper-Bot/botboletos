/*
SAINTS - RAMS FRAME COPIED FROM SUPERBOWL CHIEFS PATRIOTS - JAN 17 2019
*/
...
var start = senderId => {
  UserData.getInfo(senderId, function (err, result) {
    ...
    if (!err) {
      ...
      let buttons = [{
          type: "web_url",
          url: "www.facebook.com/fbcameraeffects/tryit/597855477327404/",
          title: "Try Saints"
        },
        {
          type: "web_url",
          url: "www.facebook.com/fbcameraeffects/tryit/1172197432944557/",
          title: "Try Rams"
        }
      ];
      ...
      Message.listButtons(senderId, title, buttons).then(() => {
        ...
        SaintsRamsButtons(senderId);
      });
    }
  });
};

var SaintsRamsButtons = senderId => {
  var replies = [{
      content_type: "text",
      title: "Get Tickets",
      payload: "SB2019"
