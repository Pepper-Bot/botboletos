...
function searchEventsByCategoryId(req, res) {
	...
	var urlApiTevo = 'https://api.ticketevolution.com/v9/events?category_id=' + category_id + '&only_with_tickets=all&occurs_at.gte=2017-10-27T13:00:00Z&occurs_at.lte=2017-12-01T13:00:00Z&order_by=events.occurs_at'
	console.log('>>>>>>>>>>>>>>>>>url tevo' + urlApiTevo);
	if (tevoClient) {
		tevoClient.getJSON(urlApiTevo).then((json) => {
