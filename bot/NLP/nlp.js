...
var PrivacyAndTermsOfService = require("../../modules/privacy_and_terms_of_service")
var NotificationsOnOff = require("../../modules/notifications_on_of")
...
var handleApiAiAction = (
  ...
  switch (action) {
    case "notifications-on-of":
      {
        NotificationsOnOff.startFavesNotificationsOnOff(sender)
      }
      break;
    case "privacy-policy":
      {
        PrivacyAndTermsOfService.startPrivacyPolicy(sender)
      }
      break;
    case "terms-of-service":
      {
        PrivacyAndTermsOfService.startTermsOfService(sender)
      }
      break;

    case "input.welcome":
      {
        Message.sendMessage(sender, responseText, action);
      }
      break;
    ...
    case "venues.nightlife.search":
      {
        ...
        if (
          ...
          contexts[0].name == "venues-nightlife" &&
          contexts[0].parameters
        ) {
          console.log("contexto " + contexts[0].name);
          zomato.zomatoStartAI(sender, contexts, action);
        } else {
          console.log("contexto " + contexts[0].name);
        }
        break;
    ...
    case "venues.eating_out.search":
      {
        ...
        if (
          isDefined(contexts[0]) &&
          contexts[0].name == "venueseating_outsearch-followup" &&
          contexts[0].parameters
        ) {
          console.log("contexto " + contexts[0].name);
    ...
    case "sharsk_tank_event":
      {
        ...
        tevoByQuery(sender, query, userPreferences).then(() => {
  ...
  if (
    ...
    if (isDefined(contexts[0].parameters.date_time)) {
      if (contexts[0].parameters.date_time != "") {
        ...
        if (finalDate == "") {
          finalDate = moment(startDate, moment.ISO_8601)
            .endOf("day")
            .format();
          finalDate = finalDate.substring(0, finalDate.length - 6);
    ...
    if (responseText === "end.events.search") {
      console.log(
        'responseText === "end.events.search"  arrayQueryMessages.length ' +
        arrayQueryMessages.length
      );
      if (arrayQueryMessages.length >= 1) {
        ...
        console.log(
          "Opps no tengo coincidencias busco por categorías ahora..."
        );
        if (event_type != "") {
          ...
          if (!err) {
            if (foundUser !== null) {
              ...
              if (isDefined(lat) && isDefined(lon)) {
                if (lat != "" && lon != "") {
                  if (date_time != "") {
                    var queryMessage = {
                      ...
                      query: tevo.API_URL +
                        "events?order_by=events.occurs_at,events.popularity_score DESC&lat=" +
                        lat +
                        "&lon=" +
                        lon +
                        "&page=1&per_page=9&" +
                ...
                console.log(
                  "no está definida la localización del usario events.search.implicit "
                );
        ...
        if (foundUser) {
          ...
          if (userSays) {
            if (userSays.typed) {
              ...
              console.log(
                "no tengo guardado lo ultimo que escribió el usuario"
              );
              //
              resolve(0);
            }
...
var startTevoByQuery = arrayQueryMessages => {
  return new Promise((resolve, reject) => {
    for (let i = 0; i < arrayQueryMessages.length; i++) {
      tevoClient
        .getJSON(arrayQueryMessages[i].query)
        .then(json => {
          let salir = false;
          if (json.error) {
            //console.log('Error al ejecutar la tevo query ' + arrayQueryMessages[i].query + 'err.message: ' + json.error);
            ...
            console.log(
              ...
              i +
              " " +
              arrayQueryMessages[i].searchBy +
              " " +
              arrayQueryMessages[i].query
            );
            if (json.events.length > 0) {
        ...
        .catch(err => {
          console.log(
            "Error al ejecutar la tevo query  " +
            arrayQueryMessages[i].query +
            "err.message: " +
            err.message
          );
