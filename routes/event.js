        ...
        if (json) {
            if (json.events.length > 0) {
                ...
                for (var i = 0, c = json.events.length; i < c; i++) {
                    // Creamos la tarjeta
                    var occurs_at = json.events[i].occurs_at;
                    ...
                    box = box.replace('{URL_PURCHASE_TICKET}', 'https://botboletos-test.herokuapp.com/tickets/?event_id=' + json.events[i].id + '&uid=' + fbId + '&venue_id=' + venue_id + '&performer_id=' + performer_id + '&event_name=' + json.events[i].name + '&event_date=' + json.events[i].occurs_at + '&seatsmap=' + json.events[i].configuration.seating_chart.large);
                    htmlHeader += box.replace('{EVENT_DESCRIPTION}', json.events[i].name + ' <br> <strong>' + moment(occurs_at).format('dddd') + ', ' + moment(occurs_at).format('MMMM Do YYYY, h:mm a') + '</strong><br>' + json.events[i].venue.name + ' ' + json.events[i].venue.location);
                    box = '';

                }

                htmlHeader += '</div> </div> </div> </div> </div> <script type="text/javascript" src="https://code.jquery.com/jquery-3.2.1.min.js"></script> <script type="text/javascript" src="https://botboletos-test.herokuapp.com/js/materialize.min.js"></script> </body></html>';
